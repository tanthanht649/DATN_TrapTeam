...
const TabButton: React.FC<{
  ...
  source: any;
  isActive: boolean;
}> = ({onPress, source, isActive}) => (
  <TouchableOpacity
    style={[_styles.tabButton, isActive && _styles.activeTabButton]}
    onPress={onPress}>
    <Image source={source} style={_styles.tabImage} />
  </TouchableOpacity>
);
...
const _ViewSwitcher: React.FC<Props> = props => {
  ...
  const text = useMemo(() => {
    return quantityEstates
      ? `${quantityEstates} tour`
    ...
    <View style={_styles.container}>
      ...
      <View style={_styles.containerBtn}>
        <TabButton
          ...
          source={
            activeTab === 'vertical' ? VERTICAL_ACTIVE : VERTICAL_INACTIVE
          }
        ...
        <TabButton
          ...
          source={
            activeTab === 'horizontal' ? HORIZONTAL_ACTIVE : HORIZONTAL_INACTIVE
          }
